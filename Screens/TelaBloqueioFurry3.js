...
export default function TelaBloqueioFurry3() {
  ...
  const initializeSystem = async () => {
    console.log('🚀 Iniciando sistema Furry3...');
    await checkServer();
    startPolling();
  };
  ...
  const checkServer = async () => {
    ...
    try {
      ...
      if (isMountedRef.current) {
        ...
        setRetryCount(0);
        console.log('✅ Servidor conectado com sucesso - Furry3');
      }
      ...
      console.log(`❌ Erro de conexão Furry3: ${error.message}`);
  ...
  const checkCard = async () => {
    ...
    try {
      ...
      const response = await fetch(`${SERVER_URL}/api/check-card/furry3`, {
        signal: controller.signal
      });
      ...
      if (data.unlocked) {
        console.log('🎉 CORUJA DETECTADA - Processando...');
        setStatus('✅ Carta da Coruja detectada! Liberando...');
        setIsConnected(true);
        ...
        setTimeout(async () => {
          try {
            ...
            console.log('🔄 Cartas resetadas após navegação da Coruja');
      ...
      console.log('❌ Erro ao verificar carta Coruja:', error.message);
  ...
  const simularLeitura = async () => {
    try {
      setStatus('🔍 Simulando leitura da Coruja...');
      
      const response = await fetch(`${SERVER_URL}/api/simulate-card/furry3`, {
        method: 'POST'
      });
      ...
      if (data.success) {
        setStatus('✅ Carta da Coruja simulada!');
      }
    } catch (error) {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.instructionsContainer}>
          ...
          <Text style={styles.subtitle}>
            Aproxime a carta física da{'\n'}
            <Text style={styles.highlight}>CORUJA</Text> no dispositivo
          </Text>
        </View>
        ...
        <View style={styles.infoBox}>
          <Text style={styles.infoText}>
            {serverOnline ? (
              `📍 ${isConnected ? 'Sistema pronto - Aproxime a carta da Coruja!' : 'Conecte o dispositivo USB'}`
            ) : (
